...
from search.filters import (SearchQueryBackend, HighlightFilterBackend,
                            LanguageFilterBackend, StoredFilterBackend)
...
class FilterTests(ElasticTestCase):
    ...
    def test_stored_filter(self):
        class StoredFilterView(SearchView):
            filter_backends = (StoredFilterBackend,)

        view = StoredFilterView.as_view()
